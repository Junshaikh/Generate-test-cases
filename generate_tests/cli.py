...
def generate_test_cases(requirement, squad, custom_filename=None, skip_upload=False, tag=None, other_tags=None, background=None, additional_background=None):
    ...
    prompt += (
        ...
        "- Consideration for Saved Addresses info on Settings screen, user see all the saved addresses in case any address saved before if not then it will be empty, user will see the option to ‘Add’ the address, on clicking ‘Add’ map will appear and user can type the location name and tap enter, after this user can click on the button ‘Enter complete address’ and on clicking it should display map view on the top of the area selected with Area name and option to save the address as ‘Apartment’ ‘House’ ‘Office’ and when selected ‘Apartment’ user has the mandatory fields to enter ‘Building name’ ‘Apt. no.’ ‘Floor’ ‘Street’ ‘Phone number beside the country flag dropdown and country code’ and optional fields to fill are Additional directions and Address label,  when selected ‘House’ user has the mandatory fields to enter ‘House’ ‘Street’ ‘Phone number beside the country flag dropdown and country code’ and optional fields to fill are ‘Floor’ ‘Additional directions’ and ‘Address label’, when selected ‘Office’ user has the mandatory fields to enter ‘Building name’ ‘Company’ ‘Floor’ ‘Street’ ‘Phone number beside the country flag dropdown and country code’ and optional fields to fill are ‘Additional directions’ and ‘Address label’,  After entering the fields user have to click on ‘Save address’  .\n”
